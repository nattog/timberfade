	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		replyFunc = OSCFunc({
			arg msg;
			var id = msg[2];
			scriptAddress.sendBundle(0, ['/engineAmpEnvelope', msg[3].asInt, msg[4].asInt, msg[5]]);
		}, path: '/ampEnvelope', srcID: context.server.addr);
		...
		2.do({
			arg i;
			players[i] = {
				arg ampEnv=0, fadeTime=0, freqRatio = 1, sampleRate, gate, playMode, voiceId, sampleId, bufnum, numFrames, startFrame, i_lockedStartFrame, endFrame, loopStartFrame, loopEndFrame;
				...
				SendReply.kr(trig: Impulse.kr(15), cmdName: '/ampEnvelope', values: [sampleId, voiceId, ampEnv]);
		...
		2.do({
			arg i;
			players[i + 2] = {
				arg ampEnv=0, fadeTime=0, freqRatio = 1, sampleRate, gate, playMode, voiceId, sampleId, bufnum, numFrames, i_lockedStartFrame, endFrame, loopStartFrame, loopEndFrame;
		...
		synthNames.do({
			...
			SynthDef(name, {
				...
				// Player
				signal = SynthDef.wrap(players[i], [\kr, \kr, \kr, \kr, \kr, \kr], [A2K.kr(ampEnvelope), fadeTime, freqRatio, sampleRate, gate, playMode]);
